...
integrators = ["TQ using SSE score (fixed), no randomness",
               "TQ using SSE score (fixed), with randomness",
               "TQ using relative SSE score, no randomness",
               "TQ using relative SSE score, with randomness"]

if __name__ == '__main__':
    plot_times(all_data, genres, font_size=17, filename_prefix='sse_split/',
               title=False, same_figure=True, integrators=integrators)
    plot_errors(all_data, genres, plot_title=False, error_bar=True, font_size=20, offset=0.1, 
                y_lim=[-200, 80], filename_prefix='sse_split/', integrators=integrators)