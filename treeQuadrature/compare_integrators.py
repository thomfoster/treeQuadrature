...
def _test_integrals_single_problem(
    ...
    for i, integral in enumerate(integrals):
        ...
        start_time = time.time()
        results, containers = integrator.integrate_containers(
            finished_containers, problem
        )
        end_time = time.time()
        if verbose >= 2:
...
def test_container_integrals(
    ...
    for problem in problems:
        ...
        for repeat in range(n_repeat):
            ...
            try:
                results = _test_integrals_single_problem(
...
def test_integrator_performance_with_params(
