...
class KernelIntegral(ContainerIntegral):
    ...
    def containerIntegral(
        ...
        if self.n_samples < 2:
            raise RuntimeError(
                "Cannot perform GP Integral with less than 2 samples"
...
class IterativeRbfIntegral(IterativeGpIntegral):
    ...
    def containerIntegral(
        ...
        f: Callable[..., np.ndarray],
        return_std: bool = False,
        previous_samples: Optional[Tuple[np.ndarray, np.ndarray]] = None,
    ):
        if self.n_samples < 2:
            raise RuntimeError(
                "Cannot perform GP Integral with less than 2 samples"
        ...
        if new_xs.shape[0] != self.n_samples:
            raise RuntimeError(
                "Number of samples do not match self.n_samples! "
        ...
        if xs.shape[0] == 0:
            raise ValueError(
                f"xs has no samples, got {n_prev} previous samples"
