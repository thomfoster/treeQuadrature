...
class GPFit(ABC):
    ...
    def create_kernel(self, gp_params: Dict[str, Any]) -> Any:
        """
        Creates and returns a kernel for the GP model.
...
class KernelFactory:
    ...
    @classmethod
    def register_kernel(cls, name: str, kernel_class: Callable):
        """
    ...
    @classmethod
    def create_kernel(cls, name: str, **kwargs):
        """
...
class SklearnGPFit(GPFit):
    ...
    def __init__(self, n_tuning: int=10, max_iter: float=1e4, factr: float=1e7, 
        ...
        # Register default kernels in the KernelFactory
        KernelFactory.register_kernel('RBF', lambda length=1.0, bounds=(1e-4, 1e4): RBF(
            length_scale=length, length_scale_bounds=bounds))

        KernelFactory.register_kernel('Polynomial', lambda degree=2, 
                                      coef0=1: Polynomial(degree=degree, coef0=coef0))
    ...
    def create_kernel(self, gp_params: dict) -> Any:
        # Get the kernel type, defaulting to 'RBF'
...
class GPyGPFit:
    """
    Implementation of GP fitting using GPy.

    Attributes
    ----------
    model : GPy.models.GPRegression
        The fitted Gaussian model. 
        Must run fit_gp before accessing model.
    mu_0 : float
        Initial mean for the log-normal prior on the length scale (RBF kernel only).
    sigma_0 : float
        Initial standard deviation for the log-normal prior on the length scale (RBF kernel only).

    Methods
    -------
    fit(xs, ys, kernel)
        Fit GP and return the model. The model will also be stored as an attribute.
    predict(xs)
        Make predictions using the fitted GP model.
    create_kernel(gp_params)
        Creates and returns a kernel for the GP model using KernelFactory.

    Example
    -------
    >>> gp = GPyGPFit()
    >>> kernel = GPy.kern.RBF(input_dim=xs_train.shape[1], lengthscale=1.0)
    >>> gp.fit(xs_train, ys_train, kernel)
    >>> ys_pred = gp.predict(xs_train)
    """

    def __init__(self, mu_0: float = 0.0, sigma_0: float = 1.0) -> None:
        """
        Arguments
        ---------
        mu_0 : float
        ...
        KernelFactory.register_kernel('RBF', 
                                      lambda lengthscale=1.0: GPy.kern.RBF(input_dim=1, 
                                                                           lengthscale=lengthscale))
        KernelFactory.register_kernel('Polynomial', lambda degree=2, 
                                      coef0=1: GPy.kern.Poly(input_dim=1, 
                                                             degree=degree, coef0=coef0))

    def fit(self, xs, ys, kernel: GPy.kern.Kern) -> GPy.models.GPRegression:
        """
        Fit a Gaussian Process model using the provided data and kernel.

        This method initializes and optimizes a Gaussian Process model 
        based on the input data `xs` and target values `ys`. If the kernel 
        is an RBF kernel, a log-normal prior is set on the lengthscale, 
        scaled by the dimensionality of the input data.

        Arguments
        ---------
        xs : array-like, shape (n_samples, n_features)
            The input data (feature matrix) to fit the Gaussian Process model.
        
        ys : array-like, shape (n_samples, 1)
            The target values corresponding to the input data.

        kernel : GPy.kern.Kern
            The kernel to be used in the Gaussian Process model. 
            If the kernel is an RBF kernel, a log-normal prior on the 
            lengthscale will be applied.
        ...
        Notes
        -----
        - If the kernel provided is of type `GPy.kern.RBF`, the lengthscale 
        of the kernel is adjusted with a log-normal prior, scaled by the 
        dimensionality of the input data (`D = xs.shape[1]`).
        - The Gaussian Process model is optimized using the `optimize` 
        method after being initialized.
        """
        D = xs.shape[1]

        # Check if the kernel is an RBF kernel
        if isinstance(kernel, GPy.kern.RBF):
            # Scale lengthscale with dimension and set a log-normal prior on the lengthscale
            mu_0_scaled = self.mu_0 + np.log(D) / 2
            kernel.lengthscale.set_prior(GPy.priors.LogNormal(mu_0_scaled, 
                                                              self.sigma_0), warning=False)

        # Create and optimize the GP model
        self.model = GPy.models.GPRegression(xs, ys, kernel)
        # Add a small jitter term for numerical stability
        self.model.likelihood.variance.fix(1e-8)
        self.model.optimize()
    ...
    def predict(self, xs, return_std: bool = False):
        ...
        """
        mean, variance = self.model.predict(xs)
    ...
    def create_kernel(self, gp_params: Dict[str, Any]) -> Any:
        ...
        gp_params : dict
            A dictionary of parameters required to create the kernel.
            For example, for RBF kernel, this may include `lengthscale`.
            For Polynomial kernel, this may include `degree` and `coef0`.
        ...
        --------
        >>> gp_fit = GPyGPFit()
        >>> kernel = gp_fit.create_kernel({'kernel_type': 'RBF', 'lengthscale': 2.0})
        >>> gp_fit.fit(xs_train, ys_train, kernel)
        """
        kernel_type = gp_params.get('kernel_type', 'RBF')

        # Create the kernel using the KernelFactory
        kernel = KernelFactory.create_kernel(kernel_type, **gp_params)

        # If the kernel is RBF, apply the log-normal prior
        if kernel_type == 'RBF':
            D = gp_params.get('input_dim', 1)
            mu_0_scaled = self.mu_0 + np.log(D) / 2
            kernel.lengthscale.set_prior(GPy.priors.LogNormal(mu_0_scaled, self.sigma_0), warning=False)
    ...
    def X_train_(self) -> np.ndarray:
        if self.model is None:
            raise RuntimeError("The Gaussian Process model is not trained. "
                               "Please call 'fit_gp' before accessing training data.")
        return self.model.X
    ...
    def hyper_params(self) -> dict:
        if self.model is None:
            raise RuntimeError("The Gaussian Process model is not trained. "
                            "Please call 'fit_gp' before accessing hyperparameters.")
        
        # Extract parameter names and values
        param_names = self.model.kern.parameter_names()
        param_values = self.model.kern.param_array
    
        # Create a dictionary mapping parameter names to values
        hyper_params_dict = {name: value for name, value in zip(param_names, param_values)}
    ...
    def lengthscale(self) -> float:
        if self.model is None:
            raise RuntimeError("The Gaussian Process model is not trained. "
                               "Please call 'fit_gp' before accessing lengthscale.")
        return self.model.kern.lengthscale
    
    @property
    def kernel_(self) -> GPy.kern.Kern:
        if self.model is None:
            raise RuntimeError(
    ...
    def alpha_(self):
        if self.model is None:
            ...
            gp = self.model
            K = gp.kern.K(gp.X) + np.eye(gp.X.shape[0]) * gp.likelihood.variance
            
            # Perform Cholesky decomposition
            L = np.linalg.cholesky(K)
            
            # Solve for alpha using the Cholesky factors
