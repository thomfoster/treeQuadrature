...
def find_neighbors_grid(
    grid: dict, container: Container, grid_size: int
) -> List[Container]:
    neighbors = []
    mins = container.mins
    maxs = container.maxs
...
class BatchGpIntegrator(TreeIntegrator):
    def __init__(
        ...
        base_grid_scale: float = 0.1,
        dimension_scaling_exponent: float = 0.9,
        length_scaling_exponent: float = 0.1,
    ...
    def _collect_results(
        self,
        leaf_containers: List[Container],
        return_N: bool,
        return_containers: bool,
        return_std: bool,
    ) -> ResultDict:
        contributions = [
            self.integral_results[container]["integral"]
            for container in leaf_containers
        ]

        return_values = ResultDict(estimate=np.sum(contributions))
        if return_N:
            return_values["n_evals"] = sum(
                [container.N for container in leaf_containers]
            )
            ...
            if samples_per_batch:
                futures = [
                    executor.submit(
                        ...
                        batch,
                        integrand,
                        samples_per_batch + (1 if i < extra_samples else 0),
                        verbose,
                        grid_size,
                        return_std,
                    )
    ...
    def _fit_batch_gp(
        ...
        iGP = IterativeGPFitting(
            ...
            gp=gp,
            fit_residuals=self.integral.fit_residuals,
        )

        if verbose:
            total_n = np.sum([cont.N for cont in batch])
            print(
                f"Fitting a batch of containers with {total_n} data points"
    ...
    def _integrate_batch(
        ...
        for container in batch:
            try:
                integral_result = kernel_integration(
                    iGP, container, gp_results, return_std
                )
            ...
            neighbors = find_neighbors_grid(
                self._build_batches(batch, grid_size), container, grid_size
            )
