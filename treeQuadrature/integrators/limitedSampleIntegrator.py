...
def save_weights_image(q):
    ...
    plt.yscale("log")
    plt.hist(ps)
    plt.savefig("/home/t/Documents/4yp/evidence-with-kdtrees/src/treeQuadrature/results/images/ps_"+str(q.n)+".png")
    plt.close()

class LimitedSampleIntegrator:

    def __init__(self, N, base_N, active_N, split, integral, weighting_function, queue=default_queue):

        """
        Integrator that builds on from queueIntegrator with more friendly controls - just keeps sampling until all samples used up. 
        """
        self.N = N
        self.base_N = base_N
    ...
    def __call__(self, problem, return_N=False, return_all=False):
        ...
        ret = (G, N, finished_containers, contributions, num_samples_left) if return_all else ret
        return ret