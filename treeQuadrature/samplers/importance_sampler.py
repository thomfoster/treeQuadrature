...
class ImportanceSampler(Sampler):
    def rvs(
        ...
        min_distance_to_edge = np.min(
            distances_to_edges, axis=1, keepdims=True)

        # Invert distances to give higher weight to
        # points closer to the edges/corners
        edge_bias = 1 / (min_distance_to_edge + 1e-6)

        # Combine edge bias with the original density
        densities = np.abs(f(samples))
        biased_probabilities = densities * edge_bias
        # Normalise to sum to 1
        biased_probabilities /= np.sum(biased_probabilities)

        # Draw samples with replacement using biased probabilities
        indices = np.random.choice(
            np.arange(n), size=n, replace=True,
            p=biased_probabilities.flatten()
        )
