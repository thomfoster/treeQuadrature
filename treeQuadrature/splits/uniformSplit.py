...
def uniformSplit(container, n_divisions):
    """
    ...
    ----------
    container : Container
        The container object to be split.
    n_divisions : int
    ...
    -------
    list
        A list containing the voxelled sub-containers.
    """
    ...
    split_points = [np.linspace(mins[i], maxs[i], n_divisions + 1) for i in range(container.D)]

    subcontainers = []

    # Iterate over all possible combinations of split points
    for indices in np.ndindex(*(n_divisions,) * container.D):
        sub_mins = [split_points[dim][indices[dim]] for dim in range(container.D)]
        sub_maxs = [split_points[dim][indices[dim] + 1] for dim in range(container.D)]
        
        # Filter points that lie within the sub-container
        mask = np.all((container.X >= sub_mins) & (container.X < sub_maxs), axis=1)
        sub_X = container.X[mask]
        sub_y = container.y[mask]
        
        # Create a new sub-container
        subcontainer = Container(sub_X, sub_y, mins=sub_mins, maxs=sub_maxs)
