    ...
    def __init__(self, estimate: float, **kwargs):
        ...
        **kwargs : Any
        ...
        if not isinstance(estimate, float):
            raise TypeError(f"'estimate' must be a float, got {type(estimate).__name__}")
    ...
    def __setitem__(self, key: str, value: Any) -> None:
        if key == 'estimate':
            if not isinstance(value, float):
                raise TypeError(f"'estimate' must be a float, got {type(value).__name__}")
        elif key == 'n_evals':
            if not isinstance(value, int):
                raise TypeError(f"'n_evals' must be an int, got {type(value).__name__}")
        elif key == 'containers':
            if not isinstance(value, list) or not all(isinstance(v, Container) for v in value):
                raise TypeError(f"'containers' must be a List[Container], got {type(value).__name__}")
        elif key == 'contributions':
            if not isinstance(value, list) or not all(isinstance(v, float) for v in value):
                raise TypeError(f"'contributions' must be a List[float], got {type(value).__name__}")
        elif key == 'stds':
            if not isinstance(value, list) or not all(isinstance(v, float) for v in value):
                raise TypeError(f"'stds' must be a List[float], got {type(value).__name__}")
        elif key == 'std':
            if not isinstance(value, float):
